...
Version 0.8.22 is a local, single-player weird-fantasy starting-region generator
...
**Combat Projection Preview** (a non-live combat-ready projection with conservative
parsing of AC, HD, movement, attacks, and damage, output to
`data/import_reports/monster_combat_projection.json` without modifying the live
catalog). The Editors hub also lists NPC, PC, Item, and Spell editor placeholders
with clear "not implemented yet" messages.
