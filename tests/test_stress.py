...
class StressTests(unittest.TestCase):
    ...
    def assert_valid_state(self, state: GameState) -> None:
        ...
        self.assertIsInstance(player.diary_entries, list)
        for entry in player.diary_entries:
            ...
            self.assertIsInstance(entry.created_day, int)
            self.assertGreaterEqual(entry.created_day, 1)
            self.assertIn(entry.created_time_period, TIME_PERIODS)
            self.assertIsInstance(entry.protected, bool)
            self.assertIsInstance(entry.hidden, bool)
        ...
        self.assertTrue(all(isinstance(lead.last_updated_day, int) for lead in player.lead_records))
